...
console.log(
  "usage: moment()",
  moment().startOf("month").startOf("week").isoWeekday(1),
);
...
const fileData = await utils
  .fileData("demo/data/echarts-package-size.json")
  .json();
...
const assetUrl = await utils.assetUrl(
